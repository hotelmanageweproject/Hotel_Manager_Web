--File này chỉ chạy SAU KHI add data vào các bảng NẾU muốn chạy trước thì phải không ADD các cột ID khóa chính VÀ ví dụ trong câu
--SELECT setval('auto_increment_booking', COALESCE((SELECT MAX(bookingid) + 1 FROM booking), 1), false);
--THÌ phải bở + 1 đi thành:
--SELECT setval('auto_increment_booking', COALESCE((SELECT MAX(bookingid) FROM booking), 1), false);
