---CUSTOMERS---
--Add customer---
CREATE or replace FUNCTION new_customer(rankid_in int, personalid_in varchar, firstname_in varchar, lastname_in varchar, birthdate_in date, gender_in char, email_in varchar, phone_in varchar, address_in varchar, OUT custadded_out BIGINT)
	RETURNS bigint 
	AS $$
	DECLARE find int;
	BEGIN
...
---
--TESTING--
select new_customer('3', '20225959', 'Khanh Nam', 'Nguyen', '2004-02-21', 'M',null, '113', 'Hanoi')
...
-----Add new booking_rooms------QUANG_TT
...
--CÁCH SỬ DỤNG HÀM
select * from new_bkrooms(700029, '0102', current_date, '2024-06-11', 2, 0) AS (bkid bigint, bookingid bigint ,roomid varchar);
...
--Tốc độ ngang nhau C2:
	explain analyze
	SELECT rs.receiptid, rs.serviceid, s.name, rs.total, rs.date, rs.staffid
	FROM room_service rs
	JOIN services s ON s.serviceid = rs.serviceid
	JOIN booking_rooms bk ON bk.bkid = rs.bkid AND bk.roomid = '0107' AND bk.bookingid = 700022
...
-------Truy vấn kiếm tra phòng hôm nay đã có sử dụng dịch vụ gì thuộc đơn đặt nào------- NAM_TT
--C1:
explain analyze
WITH subquery AS(
