...
SELECT st.staffid, dpt.name AS departmentname, st.personalid, st.firstname, st.lastname, st.birthdate,
					st.gender, st.email, st.phone, st.address, st.currentsal, st.startdate, st.enddate
...
-----------Tính số lượng bookingid trong 1 ngày cụ thể tháng năm------------HUNG TT
...
SELECT count(b.bookingid)
FROM booking b
WHERE  b.bookingdate BETWEEN '2024-01-01' AND '2024-01-31' 
...
SELECT s.paymendate,s.total_paid + SUM(rs.total) AS tong_tien
...
----Tính trung bình ngày ở của khách trong 1 năm theo tháng-------
WITH subquery AS(
	SELECT extract(day from bk.checkout) -
	FROM booking_rooms bk
	WHERE extract(year from bk.checkin) = 2024 AND extract(year from bk.checkout) = 2024
	GROUP BY month from bk.checkin
)
